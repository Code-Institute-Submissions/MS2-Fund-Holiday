...
Added a group function to run all the countries whose holidays we follow for funds with one click.
    ...
    . Having tested numerous times I've deciced to remove one of the selection boxes as I've been using the multiple selection box for everything as
     the countries we need most apear at the top and I believe having only 1 option will improve the UX.
...
Final Checks.
 ...
 Turn of Year.
    ...
    *Ran the "get all Polygon Funds button". Holidays retruend for Dec and Jan as expected. Message about no hols in Aug for Japan but again this is reading directly from the system 

    so will be ok when November comes.
